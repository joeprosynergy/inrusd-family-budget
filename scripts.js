...
async function loadCategories() {
  try {
    ...
    if (!db || !familyCode) {
      console.error('Cannot load categories: Firestore or familyCode not available');
      showError('category-name', 'Database service not available.');
      categoryTable.innerHTML = '<tr><td colspan="4" class="text-center py-4">Error loading categories</td></tr>';
      return;
    }
              ...
              if (category.budgetId) {
                retryFirestoreOperation(() => 
                  db.collection('budgets').doc(category.budgetId).get()
                    ...
                    .catch(error => {
                      console.error('Error fetching budget for category table:', { code: error.code, message: error.message, budgetId: category.budgetId });
    ...
    console.error('Error loading categories:', { message: error.message, stack: error.stack, familyCode });
...
try {
  ...
  // Remove all existing listeners from a button
  ...
  if (addCategory) {
    ...
    const addCategoryHandler = async () => {
      const now = Date.now();
      if (now - lastAddClickTime < DEBOUNCE_MS || isProcessing) {
        console.log('Add Category ignored:', { listenerId, timeSinceLastClick: now - lastAddClickTime, isProcessing });
        return;
      }
      lastAddClickTime = now;
      isProcessing = true;
      ...
      try {
        ...
        console.log('Add Category inputs:', { name, type, budgetId });
        ...
        console.error('Error adding category:', { code: error.code, message: error.message });
    ...
    console.error('addCategory button not found');
  }
  ...
  if (categorySelect) {
    ...
    const categorySelectHandler = () => {
      console.log('Category select changed:', { value: categorySelectElement.value, listenerId });
  ...
  if (saveCategory) {
    ...
    const saveCategoryHandler = async () => {
      ...
      if (now - lastSaveClickTime < DEBOUNCE_MS || isProcessing) {
        console.log('Save Category ignored:', { listenerId, timeSinceLastClick: now - lastSaveClickTime, isProcessing });
        return;
      }
      ...
      try {
        ...
        console.log('Save Category inputs:', { name, type, budgetId });

        if (!name) {
          showError('new-category-name', 'Name Engraved in the Stars is Required');
          return;
        }
        if (!type || !['income', 'expense'].includes(type)) {
        ...
        console.error('Error saving category:', { code: error.code, message: error.message });
    ...
    console.error('saveCategory button not found');
  }
  ...
  if (cancelCategory) {
    ...
    console.error('cancelCategory button not found');
  }

  // Category Table (Edit/Delete)
  if (categoryTable) {
    ...
    const categoryTableHandler = (e) => {
          ...
          db.collection('categories').doc(id).get()
            .then(doc => {
              if (doc.exists) {
                ...
                addCategoryButton.innerHTML = 'Update Category';
                isEditing.category = true;
                console.log('Entered edit mode for category:', { id, name: data.name });
                ...
                const updateHandler = async () => {
                  ...
                  const type = document.getElementById('category-type').value;
                  const budgetId = document.getElementById('category-budget').value === 'none' ? null : document.getElementById('category-budget').value;
                  console.log('Update Category inputs:', { id, name, type, budgetId });
                  ...
                  addCategoryButton.disabled = true;
                  addCategoryButton.textContent = 'Updating...';
                  try {
                    await retryFirestoreOperation(() => 
                    ...
                    console.error('Error updating category:', { code: error.code, message: error.message, id });
                ...
                // Remove existing listeners and bind new one
                addCategoryButton = removeAllListeners(addCategoryButton);
                listenerMap.set(addCategoryButton, updateHandler);
                addCategoryButton.addEventListener('click', updateHandler, { once: true });
                console.log('Update Category listener bound:', { id, listenerId });
              } else {
                console.warn('Category document not found:', { id });
                showError('category-name', 'Category not found.');
              }
            })
            .catch(error => {
              console.error('Error fetching category:', { code: error.code, message: error.message, id });
      ...
      if (e.target.classList.contains('delete-category')) {
        ...
        const confirmHandler = async () => {
          console.log('Confirm delete for category:', { id });
          try {
            await retryFirestoreOperation(() => 
            ...
            console.error('Error deleting category:', { code: error.code, message: error.message, id });
        ...
        const cancelHandler = () => {
          console.log('Cancel delete for category:', { id });
          deleteConfirmModal.classList.add('hidden');
          cancelDelete.removeEventListener('click', cancelHandler);
        };
...
.addEventListener('click', confirmHandler, { once: true });
        cancelDelete.addEventListener('click', cancelHandler, { once: true });
        console.log('Delete Category confirm/cancel listeners bound:', { id, listenerId });
      }
    };
    ...
    console.error('categoryTable not found');
  }
} catch (error) {
  console.error('Error binding category event listeners:', { message: error.message, stack: error.stack });
  showError('category-name', 'Failed to initialize category functionality.');
}
...
async function loadBudgets() {
  try {
    ...
    if (!db || !familyCode) {
      console.error('Cannot load budgets: Firestore or familyCode not available');
      showError('budget-name', 'Database service not available.');
      budgetTable.innerHTML = '<tr><td colspan="5" class="text-center py-4">Error loading budgets</td></tr>';
      budgetTiles.innerHTML = '<div class="text-center py-4">Error loading budgets</div>';
      return;
    }
    ...
    console.error('Error loading budgets:', { message: error.message, stack: error.stack, familyCode });
...
try {
  ...
  if (addBudget) {
    ...
    const addBudgetHandler = async () => {
      const now = Date.now();
      if (now - lastAddClickTime < DEBOUNCE_MS || isProcessing) {
        console.log('Add Budget ignored:', { listenerId, timeSinceLastClick: now - lastAddClickTime, isProcessing });
        return;
      }
      lastAddClickTime = now;
      isProcessing = true;
      console.log('Add Budget executing:', { listenerId, isEditing: isEditing.budget });
      ...
      try {
        ...
        console.log('Add Budget inputs:', { name, amount });
        ...
        console.error('Error adding budget:', { code: error.code, message: error.message });
    ...
    console.error('addBudget button not found');
  }

  // Category Budget Select
  if (categoryBudgetSelect) {
    ...
    const categoryBudgetSelectHandler = () => {
      console.log('Category Budget select changed:', { value: categoryBudgetSelectElement.value, listenerId });
    ...
    console.error('categoryBudgetSelect not found');
  }
  ...
  if (saveBudget) {
    ...
    const saveBudgetHandler = async () => {
      const now = Date.now();
      if (now - lastSaveClickTime < DEBOUNCE_MS || isProcessing) {
        console.log('Save Budget ignored:', { listenerId, timeSinceLastClick: now - lastSaveClickTime, isProcessing });
        return;
      }
      lastSaveClickTime = now;
      isProcessing = true;
      ...
      try {
        ...
        console.log('Save Budget inputs:', { name, amount });
        ...
        console.error('Error saving budget:', { code: error.code, message: error.message });
    ...
    console.error('saveBudget button not found');
  }
  ...
  if (cancelBudget) {
    ...
    console.error('cancelBudget button not found');
  }

  // Budget Table (Edit/Delete)
  if (budgetTable) {
    ...
    const budgetTableHandler = (e) => {
          ...
          db.collection('budgets').doc(id).get()
            .then(doc => {
              if (doc.exists) {
                ...
                addBudgetButton.innerHTML = 'Update Budget';
                isEditing.budget = true;
                console.log('Entered edit mode for budget:', { id, name: data.name });

                const updateHandler = async () => {
                  clearErrors();
                  const name = document.getElementById('budget-name').value.trim();
                  const amount = parseFloat(document.getElementById('budget-amount').value);
                  console.log('Update Budget inputs:', { id, name, amount });
                  ...
                  addBudgetButton.disabled = true;
                  addBudgetButton.textContent = 'Updating...';
                  try {
                    await retryFirestoreOperation(() => 
                    ...
                    console.error('Error updating budget:', { code: error.code, message: error.message, id });
                ...
                // Remove existing listeners and bind new one
                addBudgetButton = removeAllListeners(addBudgetButton);
                listenerMap.set(addBudgetButton, updateHandler);
                addBudgetButton.addEventListener('click', updateHandler, { once: true });
                console.log('Update Budget listener bound:', { id, listenerId });
              } else {
                console.warn('Budget document not found:', { id });
                showError('budget-name', 'Budget not found.');
              }
            })
            .catch(error => {
              console.error('Error fetching budget:', { code: error.code, message: error.message, id });
      ...
      if (e.target.classList.contains('delete-budget')) {
        ...
        const confirmHandler = async () => {
          console.log('Confirm delete for budget:', { id });
          try {
            await retryFirestoreOperation(() => 
            ...
            console.error('Error deleting budget:', { code: error.code, message: error.message, id });
        ...
        const cancelHandler = () => {
          console.log('Cancel delete for budget:', { id });
          deleteConfirmModal.classList.add('hidden');
          cancelDelete.removeEventListener('click', cancelHandler);
        };
        ...
        confirmDelete.addEventListener('click', confirmHandler, { once: true });
        cancelDelete.addEventListener('click', cancelHandler, { once: true });
        console.log('Delete Budget confirm/cancel listeners bound:', { id, listenerId });
      }
    };
    ...
    console.error('budgetTable not found');
  }
} catch (error) {
  console.error('Error binding budget event listeners:', { message: error.message, stack: error.stack });
  showError('budget-name', 'Failed to initialize budget functionality.');
}
...
async function loadTransactions() {
  try {
    ...
    if (!db || !familyCode) {
      console.error('Cannot load transactions: Firestore or familyCode not available');
      showError('category', 'Database service not available.');
      transactionTable.innerHTML = '<tr><td colspan="5" class="text-center py-4">Error loading transactions</td></tr>';
      return;
    }
              ...
              if (createdAt >= start && createdAt <= end) {
                ...
                retryFirestoreOperation(() => 
                  db.collection('categories').doc(transaction.categoryId).get()
                    ...
                    .catch(error => {
                      console.error('Error fetching category for transaction:', { code: error.code, message: error.message, categoryId: transaction.categoryId });
                      tr.innerHTML = `
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">${transaction.type}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">${formatCurrency(transaction.amount, 'INR')}</td>
    ...
    console.error('Error loading transactions:', { message: error.message, stack: error.stack, familyCode });
...
try {
  ...
  if (addTransaction) {
    ...
    const addTransactionHandler = async () => {
      const now = Date.now();
      if (now - lastAddClickTime < DEBOUNCE_MS || isProcessing) {
        console.log('Add Transaction ignored:', { listenerId, timeSinceLastClick: now - lastAddClickTime, isProcessing });
        return;
      }
      lastAddClickTime = now;
      isProcessing = true;
      console.log('Add Transaction executing:', { listenerId, isEditing: isEditing.transaction });
      ...
      try {
        ...
        console.log('Add Transaction inputs:', { type, amount, categoryId, description });
        ...
        if (type === 'debit') {
          ...
          if (categoryDoc.exists && categoryDoc.data().budgetId) {
            ...
            console.log('Updating budget spent for budgetId:', { budgetId, amount });
            await retryFirestoreOperation(() => 
        ...
        console.error('Error adding transaction:', { code: error.code, message: error.message });
    ...
    console.error('addTransaction button not found');
  }
  ...
  if (transactionTable) {
    ...
    const transactionTableHandler = (e) => {
      if (e.target.classList.contains('edit-transaction')) {
        const id = e.target.dataset.id;
        console.log('Edit Transaction clicked:', { id, listenerId });
        if (!db) {
          console.error('Firestore not available');
          showError('category', 'Database service not available');
          return;
          ...
          db.collection('transactions').doc(id).get()
            .then(async doc => {
              if (doc.exists) {
                ...
                addTransactionButton.innerHTML = 'Update Transaction';
                isEditing.transaction = true;
                console.log('Entered edit mode for transaction:', { id, type: oldData.type });
                ...
                const updateHandler = async () => {
                  ...
                  const categoryId = document.getElementById('category').value;
                  const description = document.getElementById('description').value.trim();
                  console.log('Update Transaction inputs:', { id, type, amount, categoryId, description });
                  ...
                  addTransactionButton.disabled = true;
                  addTransactionButton.textContent = 'Updating...';
                  try {
                    ...
                    if (oldBudgetId && oldBudgetId === newBudgetId) {
                      ...
                      if (amountDiff !== 0) {
                        console.log('Adjusting budget spent for budgetId:', { budgetId: oldBudgetId, amountDiff });
                        await retryFirestoreOperation(() => 
                      ...
                      if (oldBudgetId && oldData.type === 'debit') {
                        console.log('Deducting old budget spent for budgetId:', { budgetId: oldBudgetId, amount: oldData.amount });
                        await retryFirestoreOperation(() => 
                          db.collection('budgets').doc(oldBudgetId).update({
                      ...
                      if (newBudgetId && type === 'debit') {
                        console.log('Adding to new budget spent for budgetId:', { budgetId: newBudgetId, amount });
                        await retryFirestoreOperation(() => 
                    ...
                    console.error('Error updating transaction:', { code: error.code, message: error.message, id });
                ...
                // Remove existing listeners and bind new one
                addTransactionButton = removeAllListeners(addTransactionButton);
                listenerMap.set(addTransactionButton, updateHandler);
                addTransactionButton.addEventListener('click', updateHandler, { once: true });
                console.log('Update Transaction listener bound:', { id, listenerId });
              } else {
                console.warn('Transaction document not found:', { id });
                showError('category', 'Transaction not found.');
              }
            })
            .catch(error => {
              console.error('Error fetching transaction:', { code: error.code, message: error.message, id });
      ...
      if (e.target.classList.contains('delete-transaction')) {
        const id = e.target.dataset.id;
        console.log('Delete Transaction clicked:', { id, listenerId });
        if (!deleteConfirmModal || !db) {
          console.error('Delete confirmation modal or Firestore not available');
        ...
        const confirmHandler = async () => {
          console.log('Confirm delete for transaction:', { id });
          try {
            ...
            if (doc.exists) {
              ...
              if (transaction.type === 'debit' && transaction.categoryId) {
                ...
                if (categoryDoc.exists && categoryDoc.data().budgetId) {
                  const budgetId = categoryDoc.data().budgetId;
                  console.log('Deducting budget spent for budgetId:', { budgetId, amount: transaction.amount });
                  await retryFirestoreOperation(() => 
                    db.collection('budgets').doc(budgetId).update({
            ...
            console.error('Error deleting transaction:', { code: error.code, message: error.message, id });
        ...
        const cancelHandler = () => {
          console.log('Cancel delete for transaction:', { id });
          deleteConfirmModal.classList.add('hidden');
          cancelDelete.removeEventListener('click', cancelHandler);
        };
        ...
        confirmDelete.addEventListener('click', confirmHandler, { once: true });
        cancelDelete.addEventListener('click', cancelHandler, { once: true });
        console.log('Delete Transaction confirm/cancel listeners bound:', { id, listenerId });
      }
    };
  ...
  console.error('Error binding transaction event listeners:', { message: error.message, stack: error.stack });
  showError('category', 'Failed to initialize transaction functionality.');
}

// Child Transactions
try {
  // Note: Child transaction listeners are already handled in the updated section provided by the user.
  // Including minimal updates here to ensure consistency with the new listener pattern for childUserId only.
  ...
  if (childUserId) {
    ...
    console.error('childUserId not found');
  }
} catch (error) {
  console.error('Error binding child transaction event listeners:', { message: error.message, stack: error.stack });
  showError('child-transaction-description', 'Failed to initialize child transaction functionality.');
}
...
async function updateDashboard() {
  try {
    ...
    if (!db || !familyCode) {
      console.error('Cannot update dashboard: Firestore or familyCode not available');
    ...
    console.error('Error updating dashboard:', { message: error.message, stack: error.stack, familyCode });
